...
function setupDroneLaunchUI() {
    ...
    const btnSuicide = document.getElementById('btn-deploy-suicide');
    const btnAt = document.getElementById('btn-deploy-at');

    if (!launchBtn) return;

    // 드론병 선택 여부 확인 (operator면 반환, charges 상관없음)
    function getAllSelectedOperators() {
    ...
    // 발진 가능한 드론병
    function getDeployableOperators() {
        return getAllSelectedOperators().filter(u =>
    ...
    function updateDroneLaunchButton() {
        const allOps = getAllSelectedOperators();
        const deployable = getDeployableOperators();
    ...
    document.addEventListener('click', e => {
        if (popup && !popup.classList.contains('hidden')) {
            if (!launchBtn.contains(e.target) && !popup.contains(e.target)) {
