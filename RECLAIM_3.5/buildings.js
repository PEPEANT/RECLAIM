class Building extends Entity {
    constructor(type, x, y, team) {
        ...
        // [STap2] Construction state
        this.underConstruction = false;
        this.buildStartFrame = 0;
    ...
    update(enemies, players) {
        ...
        // [STap2] Under construction: tick only, no shooting/logic
        if (this.underConstruction) {
            const t = (game.frame || 0) - (this.buildStartFrame || 0);
            if (t >= (this.buildDuration || 0)) {
                this.underConstruction = false;
                // 완공 시 HP를 maxHp로 설정
                const data = CONFIG.buildings[this.type];
                if (data && data.maxHp) {
                    this.hp = data.maxHp;
    ...
    draw(ctx) {
        ...
        // ===== [STap2] Simple industrial buildings =====
        else if (
            this.type === 'supply_depot' ||
            this.type === 'barracks' ||
            this.type === 'special_barracks' ||
            this.type === 'airfield' ||
            this.type === 'research_lab' ||
            this.type === 'repair_facility' ||
            this.type === 'tank_factory'
        ) {
            const palette = {
                supply_depot: '#64748b',
                barracks: '#1f2937',
                special_barracks: '#0f172a',
                airfield: '#1e3a8a',
                research_lab: '#065f46',
                repair_facility: '#92400e',
                tank_factory: '#334155'
            };
            ctx.fillStyle = palette[this.type] || '#475569';
            ctx.fillRect(-this.width / 2, -this.height, this.width, this.height);
            ...
            ctx.fillText(this.name || this.type, 0, -this.height / 2);

            // [STap2] construction overlay
            if (this.underConstruction) {
                ctx.fillStyle = 'rgba(0,0,0,0.35)';
                ctx.fillRect(-this.width / 2, -this.height, this.width, this.height);

                const t = (game.frame || 0) - (this.buildStartFrame || 0);
                const p = Math.max(0, Math.min(1, (this.buildDuration ? (t / this.buildDuration) : 1)));

                // 진행 바 배경
                ctx.fillStyle = '#111';
                ctx.fillRect(-this.width / 2, -this.height - 12, this.width, 8);
                // 진행 바
                ctx.fillStyle = '#22c55e';
                ctx.fillRect(-this.width / 2, -this.height - 12, this.width * p, 8);

                // 진행률 텍스트
                ctx.fillStyle = '#fff';
                ctx.font = 'bold 9px sans-serif';
                ctx.textAlign = 'center';
                ctx.fillText(Math.floor(p * 100) + '%', 0, -this.height - 16);
            }
        }
