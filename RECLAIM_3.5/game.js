...
const game = {
    ...
    allowedCategories: null, // null=all, array=whitelist
    ...
    updateHUDSelection() {
        ...
        if (this.selectedBuilding && !this.selectedBuilding.dead) {
            ...
            // [STap2] 건물 실제 이름(name 속성) 사용
            const displayName = b.name || (CONFIG.buildings[b.type]?.name) || b.type || 'Building';
            HUD.setSelection({
                kind: 'building',
                name: b.type || 'Building', // 내부 식별용
                displayName: displayName,    // 표시용
                hp: b.hp || 0,
                hpMax: b.maxHp || 100,
                team: b.team || 'neutral',
    ...
    setupInputs() {
        ...
        const selectHQAt = (wx, wy) => {
            for (let b of this.buildings) {
                ...
                if (wx > b.x - b.width / 2 && wx < b.x + b.width / 2 &&
                    ...
                    // [NEW] Update HUD
                    this.updateHUDSelection();
                    // [STap2] sync category access
                    this.syncCategoryAccess();
                    return true;
                }
            }
        ...
        this.canvas.addEventListener('mousedown', e => {
            ...
            if (e.button === 2) {
                ...
                // [STap2] build placement has priority
                if (this.buildMode.active) {
                    const wx = p.x + this.cameraX;
                    const ok = this.placeBuilding(this.buildMode.buildKey, wx);
                    if (!ok) ui.showToast('여기는 건설 불가');
                    return;
                }
                // 좌클릭: 타겟팅 중이면 타겟팅 처리
        ...
        window.addEventListener('mousemove', e => {
            ...
            // [STap2] buildMode ghost 위치 갱신
            if (this.buildMode.active) {
                const wx = p.x + this.cameraX;
                this.buildMode.ghostX = Math.round(wx / 10) * 10;
                this.buildMode.ghostY = this.groundY;
                this.buildMode.valid = this.canPlaceBuilding(this.buildMode.buildKey, this.buildMode.ghostX);
            }
        ...
        this.canvas.addEventListener('touchstart', e => {
            ...
            if (e.touches.length === 1) {
                ...
                // [STap2] build placement has priority (mobile)
                if (this.buildMode.active) {
                    const wx = p.x + this.cameraX;
                    const ok = this.placeBuilding(this.buildMode.buildKey, wx);
                    if (!ok) ui.showToast('여기는 건설 불가');
    ...
    // ===== [STap2] selection -> allowed categories scaffold =====
    computeAllowedCategories() {
        const b = this.selectedBuilding;
        if (!b) return null;
        // HQ는 전부 + build
        if (b.type === 'hq_player') return ['build', 'infantry', 'armored', 'air', 'drone', 'special'];
        // 나머지는 building data에 producerCategories 있으면 그걸로 제한(일단 스캐폴드)
        const bd = (CONFIG.buildings && CONFIG.buildings[b.type]) ? CONFIG.buildings[b.type] : null;
        if (bd && Array.isArray(bd.producerCategories)) return bd.producerCategories.slice();
        return null;
    },

    syncCategoryAccess() {
        this.allowedCategories = this.computeAllowedCategories();
        // 현재 카테고리가 허용되지 않으면 첫 허용 카테고리로 이동
        if (Array.isArray(this.allowedCategories) && this.allowedCategories.length > 0) {
            if (!this.allowedCategories.includes(this.currentCategory)) {
                this.currentCategory = this.allowedCategories[0];
            }
        }
        if (typeof app !== 'undefined') app.markUiDirty();
    },

    startBuildPlacement(buildKey) {
        // HQ 선택 상태에서만 허용(자유전 기준)
        if (!this.selectedBuilding || this.selectedBuilding.type !== 'hq_player') {
            ui.showToast('총사령부 선택 후 건설 가능');
            return;
        }
        this.buildMode.active = true;
        this.buildMode.buildKey = buildKey;
        this.buildMode.valid = false;
        ui.showToast('배치 위치 선택 (우클릭/ESC 취소)');
    },

    cancelBuildPlacement() {
        if (!this.buildMode.active) return;
        this.buildMode.active = false;
        this.buildMode.buildKey = null;
        this.buildMode.valid = false;
        const ov = document.getElementById('targeting-overlay');
        if (ov) ov.classList.add('hidden');
    },

    // 겹침 체크(간단 AABB)
    canPlaceBuilding(buildKey, wx) {
        const bd = CONFIG.buildings[buildKey];
        if (!bd) return false;
        // "넓은 마당" 임시 규칙: HQ 기준 오른쪽 200~1800 구간만 허용
        const hq = this.buildings.find(b => b.type === 'hq_player');
        if (!hq) return false;
        const minX = hq.x + 200;
        const maxX = hq.x + 1800;
        ...
        for (const ob of this.buildings) {
            if (ob.dead) continue;
            const pad = 12;
            const overlap = Math.abs(ob.x - wx) < (ob.width / 2 + w / 2 + pad);
            if (overlap) return false;
        }
        return true;
    },

    placeBuilding(buildKey, wx) {
        const bd = CONFIG.buildings[buildKey];
        if (!bd) return false;
        const snappedX = Math.round(wx / 10) * 10;
        if (!this.canPlaceBuilding(buildKey, snappedX)) return false;
        const b = new Building(buildKey, snappedX, this.groundY, 'player');
        // construction
        b.underConstruction = true;
        b.buildStartFrame = this.frame || 0;
        b.buildDuration = bd.buildTime || 240;
    ...
    // [STap2] building click selection
    checkBuildingClick(wx, wy) {
        const clicked = [...this.buildings].reverse().find(b => {
            if (b.dead) return false;
            const withinX = Math.abs(b.x - wx) <= (b.width / 2 + 6);
            const top = b.y - b.height;
            const withinY = (wy >= top && wy <= b.y);
            // 아군만 선택(원하면 중립/적도 가능)
            return withinX && withinY && b.team === 'player';
        });
        if (clicked) {
    ...
    draw() {
        ...
        // ===== [STap2] build ghost =====
        if (this.buildMode.active && this.buildMode.buildKey) {
            const bd = CONFIG.buildings[this.buildMode.buildKey];
            if (bd) {
                ctx.save();
                ctx.globalAlpha = 0.55;
                ctx.fillStyle = this.buildMode.valid ? '#22c55e' : '#ef4444';
                ctx.fillRect(
                    this.buildMode.ghostX - bd.width / 2,
                    this.groundY - bd.height,
                    bd.width,
                    bd.height
                );
                // 건물 이름 표시
                ...
                ctx.fillText(bd.name || this.buildMode.buildKey, this.buildMode.ghostX, this.groundY - bd.height / 2);
                ctx.restore();
            }
        }
