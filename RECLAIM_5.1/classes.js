...
class Unit extends Entity {
    ...
    draw(ctx) {
        ...
        if (id === 'worker') {
            ...
            // [PARAM] blackhawk: UNIT_PARAMS reference
            const p = (typeof UNIT_PARAMS !== 'undefined' && UNIT_PARAMS.blackhawk) || {};
            const body = p.body || { scaleX: 1, scaleY: 1 };
            const cockpit = p.cockpit || { x: -10, y: -5, w: 25, h: 12 };
            const mainRotor = p.mainRotor || { x: -5, y: -12, w: 90, h: 4 };
            const tailRotor = p.tailRotor || { x: -35, y: -15, w: 2, h: 20, speedMul: 3 };
            // Body (6-point polygon with scale)
            ctx.save();
            ctx.scale(body.scaleX, body.scaleY);
            ctx.fillStyle = '#111827';
            ctx.beginPath();
            ctx.moveTo(15, -5); ctx.lineTo(-25, -5); ctx.lineTo(-35, -15);
            ctx.lineTo(-25, 5); ctx.lineTo(10, 10); ctx.lineTo(20, 5);
            ctx.fill();
            ctx.restore();
            ...
            // Main Rotor
            ctx.save();
            ctx.translate(mainRotor.x, mainRotor.y);
            ctx.fillStyle = '#000';
