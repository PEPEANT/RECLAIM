 ...
 * DEFAULT_UNIT_SHAPES
 * 각 유닛의 기본 폴리곤 점 데이터
 * classes.js의 draw() 코드에서 추출하여 단순화한 외곽선
 */
const DEFAULT_UNIT_SHAPES = {
  ...
  worker: {
    ...
    layers: [
      // 몸통 (노란색 조끼)
      { name: "Body", color: "#facc15", points: [
        { x: -7, y: -22 }, { x: 7, y: -22 }, { x: 7, y: 0 }, { x: -7, y: 0 }
      ]},
      // 머리
      { name: "Head", color: "#3b82f6", points: [
        { x: -5, y: -31 }, { x: 5, y: -31 }, { x: 5, y: -21 }, { x: -5, y: -21 }
      ]},
      // 헬멧
      { name: "Helmet", color: "#f59e0b", points: [
        { x: -6, y: -33 }, { x: 6, y: -33 }, { x: 6, y: -27 }, { x: -6, y: -27 }
      ]},
      // 도구
      { name: "Tool", color: "#64748b", points: [
  ...
  rpg: {
    ...
    layers: [
      ...
      // RPG 발사관
      { name: "Launcher", color: "#334155", points: [
        { x: -2, y: -24 }, { x: 10, y: -24 }, { x: 10, y: -18 }, { x: -2, y: -18 }
      ]},
      // 탄두
      { name: "Warhead", color: "#7f1d1d", points: [
  ...
  special_forces: {
    ...
    layers: [
      // 몸통 하단
      { name: "BodyLower", color: "#171717", points: [
        { x: -7, y: -12 }, { x: 7, y: -12 }, { x: 7, y: 0 }, { x: -7, y: 0 }
      ]},
      // 몸통 상단 (조끼)
      { name: "BodyUpper", color: "#1e293b", points: [
        { x: -7, y: -22 }, { x: 7, y: -22 }, { x: 7, y: -12 }, { x: -7, y: -12 }
      ]},
      // 머리
      { name: "Head", color: "#171717", points: [
        { x: -5, y: -31 }, { x: 5, y: -31 }, { x: 5, y: -21 }, { x: -5, y: -21 }
      ]},
      // 야간투시경
      { name: "NVG", color: "#10b981", points: [
        { x: -4, y: -28 }, { x: 4, y: -28 }, { x: 4, y: -25 }, { x: -4, y: -25 }
      ]},
      // 총
      { name: "Gun", color: "#000000", points: [
  ...
  drone_operator: {
    ...
    layers: [
      // 백팩
      { name: "Backpack", color: "#334155", points: [
  ...
  humvee: {
    ...
    layers: [
      ...
      // 창문/캐빈
      { name: "Cabin", color: "#1e293b", points: [
        { x: -10, y: -15 }, { x: -5, y: -25 }, { x: 10, y: -25 }, { x: 15, y: -15 }
      ]},
      // 앞바퀴
      { name: "FrontWheel", color: "#000000", points: [
        { x: -18, y: -6 }, { x: -6, y: -6 }, { x: -6, y: 6 }, { x: -18, y: 6 }
      ]},
      // 뒷바퀴
      { name: "RearWheel", color: "#000000", points: [
  ...
  apc: {
    ...
    layers: [
      ...
      // 포탑
      { name: "Turret", color: "#1e293b", points: [
        { x: -15, y: -22 }, { x: 15, y: -22 }, { x: 15, y: -16 }, { x: -15, y: -16 }
      ]},
      // 바퀴들
      { name: "Wheels", color: "#000000", points: [
        { x: -18, y: -6 }, { x: -6, y: -6 }, { x: -6, y: 6 }, { x: -18, y: 6 }
      ]}
    ]
  },
  ...
  aa_tank: {
    ...
    layers: [
      ...
      // 대공포 (좌)
      { name: "GunLeft", color: "#1e293b", points: [
        { x: -4, y: -34 }, { x: 0, y: -34 }, { x: 0, y: -22 }, { x: -4, y: -22 }
      ]},
      // 대공포 (우)
      { name: "GunRight", color: "#1e293b", points: [
        { x: 4, y: -34 }, { x: 8, y: -34 }, { x: 8, y: -22 }, { x: 4, y: -22 }
      ]},
      // 무한궤도
      { name: "Tracks", color: "#000000", points: [
  ...
  mbt: {
    ...
    layers: [
      ...
      // 주포
      { name: "MainGun", color: "#1e293b", points: [
  ...
  spg: {
    ...
    layers: [
      ...
      // 포신 (45도 각도)
      { name: "Barrel", color: "#1e293b", points: [
  ...
  fighter: {
    ...
    layers: [
      // 동체
      { name: "Fuselage", color: "#3b82f6", points: [
        { x: 32, y: -8 }, { x: -16, y: -14 }, { x: -24, y: -8 }, { x: -16, y: -2 }
      ]},
      // 날개
      { name: "Wing", color: "#1e293b", points: [
  ...
  apache: {
    ...
    layers: [
      // 꼬리
      { name: "Tail", color: "#334155", points: [
        { x: -40, y: -5 }, { x: -10, y: -5 }, { x: -10, y: 3 }, { x: -40, y: 3 }
      ]},
      // 본체
      { name: "Body", color: "#1e3a8a", points: [
      ...
      // 로터
      { name: "Rotor", color: "#000000", points: [
  ...
  blackhawk: {
    ...
    layers: [
      // 본체
      { name: "Body", color: "#111827", points: [
        { x: 15, y: -5 }, { x: -25, y: -5 }, { x: -35, y: -15 },
        { x: -25, y: 5 }, { x: 10, y: 10 }, { x: 20, y: 5 }
      ]},
      // 콕핏
      { name: "Cockpit", color: "#0f172a", points: [
        { x: -10, y: -5 }, { x: 15, y: -5 }, { x: 15, y: 7 }, { x: -10, y: 7 }
      ]},
      // 메인 로터
      { name: "MainRotor", color: "#000000", points: [
  ...
  chinook: {
    ...
    layers: [
      // 본체
      { name: "Body", color: "#4b5563", points: [
        { x: -30, y: -10 }, { x: 30, y: -10 }, { x: 35, y: 5 },
        { x: -35, y: 5 }, { x: -35, y: -20 }
      ]},
      // 랜딩기어 (앞)
      { name: "GearFront", color: "#1f2937", points: [
        { x: -25, y: 5 }, { x: -17, y: 5 }, { x: -17, y: 9 }, { x: -25, y: 9 }
      ]},
      // 랜딩기어 (뒤)
      { name: "GearRear", color: "#1f2937", points: [
        { x: 15, y: 5 }, { x: 23, y: 5 }, { x: 23, y: 9 }, { x: 15, y: 9 }
      ]},
      // 전방 로터
      { name: "FrontRotor", color: "#000000", points: [
        { x: -75, y: -22 }, { x: 5, y: -22 }, { x: 5, y: -18 }, { x: -75, y: -18 }
      ]},
      // 후방 로터
      { name: "RearRotor", color: "#000000", points: [
  ...
  bomber: {
    ...
    layers: [
      // 동체
      { name: "Fuselage", color: "#334155", points: [
        { x: 60, y: 0 }, { x: 40, y: -7 }, { x: -20, y: -7 },
        { x: -40, y: -25 }, { x: -35, y: -5 }, { x: -50, y: 0 },
        { x: -45, y: 5 }, { x: 20, y: 8 }
      ]},
      // 콕핏
      { name: "Cockpit", color: "#000000", points: [
        { x: 40, y: -7 }, { x: 50, y: -2 }, { x: 42, y: -2 }
      ]},
      // 날개
      { name: "Wings", color: "#1e293b", points: [
        { x: 10, y: -2 }, { x: -30, y: -2 }, { x: -40, y: 15 }, { x: -10, y: 15 }
      ]},
      // 팀 마크
      { name: "TeamMark", color: "#3b82f6", points: [
  ...
  recon: {
    ...
    layers: [
      // 동체
      { name: "Fuselage", color: "#cbd5e1", points: [
        { x: 30, y: 5 }, { x: 10, y: -12 }, { x: -40, y: -5 },
        { x: -35, y: 2 }, { x: 20, y: 10 }
      ]},
      // 날개
      { name: "Wings", color: "#94a3b8", points: [
        { x: -5, y: -5 }, { x: -25, y: -5 }, { x: -35, y: 25 }, { x: 5, y: 25 }
      ]},
      // 꼬리 날개
      { name: "TailWing", color: "#64748b", points: [
        { x: -35, y: -5 }, { x: -45, y: -25 }, { x: -40, y: -5 }
      ]},
      // 센서/카메라
      { name: "Sensor", color: "#1e293b", points: [
  ...
  drone_suicide: {
    ...
    layers: [
      // 본체
      { name: "Body", color: "#475569", points: [
        { x: -12, y: -8 }, { x: 12, y: -8 }, { x: 12, y: 4 }, { x: -12, y: 4 }
      ]},
      // 팀 컬러 띠
      { name: "TeamStripe", color: "#3b82f6", points: [
        { x: -12, y: -2 }, { x: 12, y: -2 }, { x: 12, y: 1 }, { x: -12, y: 1 }
      ]},
      // 로터 암
      { name: "RotorArm", color: "#1e293b", points: [
        { x: -20, y: -4 }, { x: 20, y: -4 }, { x: 20, y: 0 }, { x: -20, y: 0 }
      ]},
      // 폭발물
      { name: "Explosive", color: "#ef4444", points: [
  ...
  drone_at: {
    ...
    layers: [
      // 동체
      { name: "Body", color: "#e2e8f0", points: [
        { x: 20, y: 2 }, { x: 0, y: -10 }, { x: -20, y: -2 },
        { x: -20, y: 2 }, { x: 0, y: 8 }
      ]},
      // 꼬리 날개 (상)
      { name: "TailUp", color: "#94a3b8", points: [
        { x: -15, y: -2 }, { x: -25, y: -12 }, { x: -20, y: -2 }
      ]},
      // 꼬리 날개 (하)
      { name: "TailDown", color: "#94a3b8", points: [
        { x: -15, y: 2 }, { x: -25, y: 12 }, { x: -20, y: 2 }
      ]},
      // 대전차 미사일
      { name: "Missile", color: "#dc2626", points: [
  ...
  stealth_drone: {
    ...
    layers: [
      ...
      // 센서
      { name: "Sensor", color: "#0f172a", points: [
  ...
  emp: {
    ...
    layers: [
      // 외곽 원
      { name: "Ring", color: "#3b82f6", points: [
        { x: -15, y: -15 }, { x: 15, y: -15 }, { x: 15, y: 15 }, { x: -15, y: 15 }
      ]},
      // 번개 모양
      { name: "Lightning", color: "#3b82f6", points: [
  ...
  nuke: {
    ...
    layers: [
      // 외곽 원
      { name: "Circle", color: "#ef4444", points: [
        { x: -15, y: -15 }, { x: 15, y: -15 }, { x: 15, y: 15 }, { x: -15, y: 15 }
      ]},
      // 중심
      { name: "Core", color: "#facc15", points: [
  ...
  tactical_missile: {
    ...
    layers: [
      // 동체
      { name: "Body", color: "#e5e7eb", points: [
        { x: -12, y: -3 }, { x: 12, y: -3 }, { x: 12, y: 3 }, { x: -12, y: 3 }
      ]},
      // 노즈콘
      { name: "Nose", color: "#ef4444", points: [
        { x: 12, y: -3 }, { x: 18, y: 0 }, { x: 12, y: 3 }
      ]},
      // 꼬리 날개 (상)
      { name: "FinTop", color: "#475569", points: [
        { x: -8, y: -3 }, { x: -12, y: -8 }, { x: -12, y: -3 }
      ]},
      // 꼬리 날개 (하)
      { name: "FinBottom", color: "#475569", points: [
        { x: -8, y: 3 }, { x: -12, y: 8 }, { x: -12, y: 3 }
      ]},
      // 화염
      { name: "Flame", color: "#f59e0b", points: [
