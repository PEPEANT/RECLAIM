...
## 7. 최근 수정 사항 (2026-02-02)

### 7.1 버그 수정 완료

#### 버그 1: 보병 데미지 불일치 (projectiles.js)
- **문제**: hit() 호출 조건(30px)과 데미지 적용 조건(22.5px) 불일치
- **수정**: bullet/machinegun 타입의 hit 판정 거리를 20px로 조정
- **파일**: `projectiles.js:130-132`

#### 버그 2: 뉴스 카메라 포커스 (news_overlay.js)
- **문제**: 10초 후 적이 민간인 근처에 없으면 하늘만 표시
- **수정**:
  - 1순위: 적이 민간인을 실제로 공격 중인 위치
  - 2순위: 가장 진격한 적 유닛 위치
  - "뉴스 카메라 포착" 캡션 + REC 표시 추가
- **파일**: `news_overlay.js:421-576`

### 7.2 최적화 완료

#### 배열 복사 제거 (GC 부하 감소)
- **문제**: 전술미사일/핵 피해 계산 시 매번 새 배열 복사
- **수정**: 스프레드 연산자 대신 직접 forEach 순회
- **파일**: `projectiles.js:154-170, 215-226`

#### 메모리 누수 방지
- **문제**: setTimeout/addEventListener 미정리
- **수정**: `game._cleanupTimers()` 메서드 추가, endGame()에서 호출
- **파일**: `game.js:2957-2976`
...
│   └── camera.js (기존)
├── data/
│   └── config.js (향후 CONFIG 분리용)
├── entities/
│   └── entity.js (Entity 클래스 분리)
├── systems/
│   └── (향후 AI, combat 등 분리)
├── ui/
│   └── (향후 HUD, news 등 분리)
└── vfx/
    ├── explosion.js (기존)
    └── particle.js (Particle, SmokeCloudFX 분리)
```
...
| 우선순위 | 문제 | 파일 |
|---------|------|------|
| **High** | O(n²) 클러스터 계산 | ai.js:168-200 |
| **Medium** | 드론 상태 동기화 불일치 | classes.js |
| **Medium** | 추가 배열 복사 최적화 | projectiles.js, classes.js |

### 7.5 파일 분리 마이그레이션 가이드

#### 안전한 분리 순서
1. Entity 클래스 (완료: `src/entities/entity.js`)
2. Particle 클래스 (완료: `src/vfx/particle.js`)
3. CONFIG 데이터 (준비됨: `src/data/config.js`)
4. Projectile 클래스 → `src/entities/projectile.js`
5. Unit 클래스 → `src/entities/unit.js`
6. Building 클래스 → `src/entities/building.js`
7. AI 시스템 → `src/systems/ai.js`

#### index.html 스크립트 로드 순서 (분리 완료 시)
```html
<!-- 1. 데이터 -->
...
<script src="src/vfx/particle.js"></script>
...
<script src="news_overlay.js"></script>
