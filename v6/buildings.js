...
class Building extends Entity {
    ...
    draw(ctx) {
        ...
        if (this.type.includes('hq')) {
            ...
            // City defense line (visual + blocker)
            ...
            const mapsApi = (typeof Maps !== 'undefined' && Maps) ? Maps : null;
            const mapHook = (hookName) => {
                if (mapsApi && typeof mapsApi.getMapHook === 'function') {
                    return mapsApi.getMapHook(hookName, 'city');
                }
                if (typeof window !== 'undefined'
                    && window.MapRegistry
                    && typeof window.MapRegistry.getMapHook === 'function') {
                    return window.MapRegistry.getMapHook('city', hookName);
                }
                return null;
            };
            const drawLuxuryFn = mapHook('drawLuxuryApartment')
                || (mapsApi && mapsApi.drawLuxuryApartment);
            const drawLegacyFn = mapHook('drawLegacyApartment')
                || (mapsApi && mapsApi.drawLegacyApartment);
            const drawDestroyedLuxuryFn = mapHook('drawDestroyedLuxuryApartment')
                || (mapsApi && mapsApi.drawDestroyedLuxuryApartment);
            const drawDestroyedLegacyFn = mapHook('drawDestroyedLegacyApartment')
                || (mapsApi && mapsApi.drawDestroyedLegacyApartment);
            ...
            const hasCityDesign = isCity
                && (cityVariant === 'luxury' || cityVariant === 'legacy')
                && (typeof drawLuxuryFn === 'function')
                && (typeof drawLegacyFn === 'function');

            if (hasCityDesign) {
                const baseW = cityVariant === 'luxury' ? 100 : 80;
                const baseH = 150;
                const scale = Math.max(0.6, (this.width || 80) / baseW);

                // [NEW] 파괴된 건물 렌더링
                if (this.isDestroyed) {
                    const drawDestroyedFn = (cityVariant === 'luxury')
                        ? drawDestroyedLuxuryFn
                        : drawDestroyedLegacyFn;
                    if (drawDestroyedFn) {
                        drawDestroyedFn(ctx, 0, 0, scale);
                    } else {
                        drawDestroyedFallback(scale);
                    }

                    ctx.restore();
                    return;  // 파괴된 건물은 HP바 등 표시 안함
                }

                const drawFn = (cityVariant === 'luxury') ? drawLuxuryFn : drawLegacyFn;
                drawFn(ctx, 0, 0, scale);

                const barW = baseW * scale;
                const barH = Math.max(4, 6 * scale);
                const barY = -baseH * scale - 10 * scale;
                const teamColor = this.team === 'neutral' ? '#64748b' : (this.team === 'player' ? '#3b82f6' : '#ef4444');

                // [NEW] 중립일 때만 점령 바 표시, 점령된 건물은 체력 바만 표시
                if (this.team === 'neutral') {
                    // 점령 바 (중립 상태) - 점령 진행 중일 때만 표시
                    if (this.captureProgress !== 0) {
                        ctx.fillStyle = '#0f172a';
                        ctx.fillRect(-barW / 2, barY, barW, barH);
                        if (this.captureProgress > 0) {
                            ctx.fillStyle = '#3b82f6';
                            ctx.fillRect(-barW / 2, barY, barW * (this.captureProgress / 100), barH);
                        } else if (this.captureProgress < 0) {
                            ctx.fillStyle = '#ef4444';
                            const startX = (barW / 2) + (barW * (this.captureProgress / 100));
                            const width = -barW * (this.captureProgress / 100);
                            ctx.fillRect(startX, barY, width, barH);
                        }
                    }
                } else {
                    // 체력 바 (점령된 상태)
                ...
                ctx.fillRect(-barW / 2, -4 * scale, barW, 4 * scale);

                // [NEW] 주둔 보병 수 표시 (건물 선택 시 또는 주둔 유닛 있을 때)
                const garrisonCount = this.garrisonUnits ? this.garrisonUnits.length : 0;
                const isSelected = (typeof game !== 'undefined' && game.selectedBuilding === this);
                if (garrisonCount > 0 || isSelected) {
                    // 주둔 보병 수 UI (프로필 스타일)
                    const infoY = barY - 25 * scale;
                    const infoW = 60 * scale;
                    const infoH = 20 * scale;

                    // 배경
                    ctx.fillStyle = 'rgba(15, 23, 42, 0.9)';
                    ctx.fillRect(-infoW / 2, infoY, infoW, infoH);
                    ctx.strokeStyle = teamColor;
                    ctx.lineWidth = 1;
                    ctx.strokeRect(-infoW / 2, infoY, infoW, infoH);
                    ...
                    ctx.fillText(`🪖 ${garrisonCount}/${this.maxGarrison}`, 0, infoY + infoH / 2);
                    ...
                    if (garrisonCount > 0) {
                        ...
                        ctx.fillText(`+${defBonus}% DEF`, 0, infoY + infoH + 8 * scale);
                    }
                }
            } else {
                if (this.isDestroyed) {
                ...
                if (this.team === 'neutral') {
                    ...
                    if (this.captureProgress !== 0) {
                        ...
                        else if (this.captureProgress < 0) { ctx.fillStyle = '#ef4444'; ctx.fillRect(40 + (80 * (this.captureProgress / 100)), -70, -80 * (this.captureProgress / 100), 6); }
                    }
                } else {
                    // 체력 바 (점령된 상태)
                    const hpRatio = Math.max(0, Math.min(1, this.hp / this.maxHp));
