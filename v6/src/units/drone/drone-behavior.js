...
const DroneBehavior = {
    update(drone, enemies, buildings) {
        ...
        // [FIX] 드론병 사망 시 드론 즉시 제거 (orphan drone 방지)
    ...
    findNearestEnemy(drone, enemies, buildings) {
        ...
        // [CHANGE] 유닛과 건물 중 가까운 것을 타겟 (건물도 함께 검색)
        if (buildings && buildings.length) {
            // 적 건물 중 가장 가까운 것 찾기
            let nearestBuilding = null;
            let nearestBuildingSq = 999999999;
            ...
            for (let i = 0; i < buildings.length; i++) {
                ...
                const dx = b.x - x;
                const dSq = dx * dx;
                if (dSq < nearestBuildingSq) {
                    nearestBuildingSq = dSq;
                    nearestBuilding = b;
                }
            }

            // [NEW] 건물이 유닛보다 가까우면 건물을 타겟으로
            if (nearestBuilding && nearestBuildingSq < minSq) {
                t = nearestBuilding;
                minSq = nearestBuildingSq;
            }
            // 유닛이 없고 건물이 있으면 건물을 타겟으로
            else if (!t && nearestBuilding) {
                t = nearestBuilding;
            }
        }
        return t;
