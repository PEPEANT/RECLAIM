...
(function attachUnitRenderV2State_sniper(globalScope) {
    ...
    function ensureState(unit) {
        ...
        if (!unit._renderV2State.sniper || typeof unit._renderV2State.sniper !== 'object') {
            unit._renderV2State.sniper = Object.assign({}, DEFAULT_STATE);
        }
        return unit._renderV2State.sniper;
    }
    ...
    function updateState(unit, state) {
        ...
        if (Number.isFinite(attackFrame) && attackFrame > 0 && attackFrame !== prevAttackFrame) {
            state.recoil = Math.max(Number(state.recoil) || 0, 3.6);
            state.muzzleFlash = Math.max(Number(state.muzzleFlash) || 0, 5);
        }

        state.weaponSway = (Number(state.weaponSway) || 0) * 0.66;
        state.torsoLean = (Number(state.torsoLean) || 0) * 0.72;
        state.legSwing = (Number(state.legSwing) || 0) * 0.9;
        state.armSwing = (Number(state.armSwing) || 0) * 0.7;
