...
(function attachUnitRenderV2Body_sniper(globalScope) {
    ...
    function drawGripToWeapon(ctx, palette, state, recoil) {
        ...
        var baseX = 2 - (Number(recoil) || 0) * 0.45 + bobX;
        var baseY = -20 - bobY;
        var ang = -0.04 + sway * 0.6;

        var supportX = baseX + Math.cos(ang) * 7.0;
        var supportY = baseY + Math.sin(ang) * 7.0;
        var gripX = baseX + Math.cos(ang) * 3.6;
        var gripY = baseY + Math.sin(ang) * 3.6;
