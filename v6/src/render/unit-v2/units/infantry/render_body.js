...
(function attachUnitRenderV2Body_infantry(globalScope) {
    ...
    function drawHead(ctx, x, y, palette) {
        ctx.save();
        ctx.translate(x, y);
    ...
    function drawStanding(ctx, palette, state, recoil) {
        ...
        ctx.save();
        ctx.translate(0, -bodyBob - (idleBreath * 0.25));
    ...
    function drawCrouching(ctx, palette, state, recoil) {
        ...
        drawHead(ctx, 0, -23 + (idleBreath * 0.2), palette);
    ...
    function drawBody(unit, ctx, state, palette) {
        ...
        if (stance === 'prone') {
            drawProne(ctx, palette, state, recoil);
        } else if (stance === 'crouching') {
            drawCrouching(ctx, palette, state, recoil);
        } else {
            drawStanding(ctx, palette, state, recoil);
        }
    }
