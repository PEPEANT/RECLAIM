// Per-unit runtime render state for: drone_operator
(function attachUnitRenderV2State_drone_operator(globalScope) {
    'use strict';
    ...
    function ensureState(unit) {
        ...
        if (!unit._renderV2State.drone_operator || typeof unit._renderV2State.drone_operator !== 'object') {
            unit._renderV2State.drone_operator = Object.assign({}, DEFAULT_STATE);
        }
        return unit._renderV2State.drone_operator;
    }
    ...
    function updateState(unit, state) {
        ...
        state.mode = (unit.opState === 'laptop') ? 'laptop' : 'rifle';

        if (state.mode === 'laptop') {
            state.recoil = 0;
            state.muzzleFlash = 0;
            state.weaponSway = (Number(state.weaponSway) || 0) * 0.4;
            state.legSwing = (Number(state.legSwing) || 0) * 0.6;
            if (unit.commandMode === 'stop') state.stance = 'crouching';
            return;
        }
    ...
    globalScope['UnitRenderV2State_drone_operator'] = {
        getState: getState,
