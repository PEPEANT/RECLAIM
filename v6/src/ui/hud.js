 ...
 *
 * ??????????
 * - HUD??"??癲?嶺?+ ?嶺???????꿔꺂??濡⑷괌?遺쎄큿??걜?????????
 * - ???濚밸Ŧ?김???????산뭐鶯?? ??????????癲?嶺??game, unit_commands)????????彛?
 * - ????節떷????????game 1?? HUD????癲?嶺뚮쵓???
 */
...
const HUD = {
    ...
    executeAllMoveCommand() {
        ...
        if (typeof game.clearAllDirectControlReleaseHold === 'function') {
            game.clearAllDirectControlReleaseHold();
        }
    ...
    executeAllRetreatCommand() {
        ...
        if (issued > 0 && typeof ui !== 'undefined' && ui && typeof ui.showToast === 'function') {
            const recallSuffix = recalled > 0 ? ` / 드론 회수 ${recalled}` : '';
            const scopeText = (targetCtx.scope === 'all') ? '전군' : '선택 유닛';
            ui.showToast(`${scopeText} 후퇴 명령 (${issued}${recallSuffix})`);
        }
        return issued > 0;
    },
    ...
    executeAllStopCommand() {
        ...
        if (targetCtx.scope === 'direct'
            && typeof game.isDirectControlActive === 'function'
            && game.isDirectControlActive()) {
            const directUnit = (typeof game.getDirectControlUnit === 'function')
                ? game.getDirectControlUnit()
                : null;
            if (directUnit && typeof game.setDirectControlReleaseHold === 'function') {
                game.setDirectControlReleaseHold(directUnit, true);
            }
            // Keep direct-control active; "all stop" should not force control release.
            if (typeof game.clearDirectControlKeys === 'function') {
                game.clearDirectControlKeys();
            }
        }
    ...
    getCommandContext() {
        ...
        selectedUnits.forEach(u => {
            ...
            // [ITEM] ??⑤벡異?? special_forces ?リ옇?????蹂ㅽ깴, smokeChargesLeft > 0??嶺뚮ㅄ維獄???ル봾六?嶺뚯솘???
            if ((u.smokeChargesLeft || 0) > 0) hasSmokeCharge = true;
            // [ITEM] ??濡?뵹 ???덈콦: medkitChargesLeft > 0???뺢퀣伊?????ル봾六?
    ...
    setSelection(selection) {
        ...
        if (selection.kind === 'unit') {
            ...
            if (isHQ) {
                ...
                // [NEW] ?嶺????縕???轅몄뫅????? ????節떷????????용츧?嶺뚮??껆빊????ㅼ뒧??????꿔꺂?????????癲?嶺?
    ...
    // [?轅붽틓??熬곥끇釉??룸???C/D] HQ ????節떷?????????ル뭸癲???????밸븶???production area???????밸븶???
    // worker??infantry ???ㅳ늾???雅?퍔瑗?????숈?????????癲ル슢??????????ル뭸癲????????袁⑸즴??????ル봿????
    ...
    getSelectedProductionBuilding() {
        ...
        const b = game.selectedBuilding;
        // canProduce ???????釉????? ??????뀀땽 ?轅몄뫅????轅롫쳳???(???ㅼ뒧??????ル∥堉?嚥싲갭횧??? ?????밸븶筌먯죾??節뗭젡??ㅻぜ??됱춻?裕뼘?)
    ...
    showProductionBuildingUI(building, productionArea, footer, buildingLabel) {
        ...
        // ??????????????ル뭸癲??轅붽틓??熬곥끇釉띄춯誘좊??????ル봿????꿔꺂????紐꾩뮏?β뼯猷???
    ...
    showBuildButtons(productionArea, footer, buildingLabel) {
        ...
        for (const key in buildings) {
            ...
            const alreadyBuilt = game.watchtowerBuilt;  // [3.8] 1???轅몄뫅????????????????轅붽틓??????⒟?
            const isDisabled = !canAfford || onCooldown || alreadyBuilt;

            // [3.8] btn-unit ????????????????(?????ル뭸癲ル슢理???쒗???쇰뮝力?????읐??????⑤베???????繹??
            ...
            ctx.save();
            ctx.translate(30, 38);
            ctx.scale(0.16, 0.16);  // ????????곗뒭????
            // ????????watchtower ????거???????????(buildings.js ?轅붽틓???????
            ctx.fillStyle = '#555';  // ?????????留?
            ctx.fillRect(-25, -150, 50, 150);
            ctx.fillStyle = '#444';  // ??ш끽維뽳쭩??룸챶????
            ctx.fillRect(-45, -150, 90, 10);
            ctx.fillStyle = '#111';  // ?????????
            ctx.fillRect(25, -185, 35, 6);
            ctx.fillStyle = '#666';  // ?嶺????縕?????ㅼ뒧?戮レ땡?塋??
            ctx.fillRect(-40, -210, 80, 60);
            ctx.fillStyle = '#333';  // ?????遊붋耀붾굛?????ш끽維??λ궔????洹욌춴?믨틵???
            ctx.fillRect(20, -220, 20, 70);
            ctx.fillStyle = '#444';  // ?轅붽틓?????
     ...
     */
    hideLegacyUI() {
        // [3.8] Hide old minimap/toggle/ctrl/cmd buttons (???濚밸Ŧ????嶺?????? ???)
