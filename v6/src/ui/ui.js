...
const ui = {
    ...
    updateUnitButtons(cat, stock, cooldowns, supply, queue) {
        ...
        const icbmSkillKeys = new Set(['nuke', 'tactical_missile', 'emp']);
        const requestedCategory = (cat === 'infantry' || cat === 'armored' || cat === 'air')
            ? cat
            : 'infantry';
        const coastLimitedRosterMode = !!(
            typeof game !== 'undefined'
            && game
            && typeof game.isCoastLimitedRosterMode === 'function'
            && game.isCoastLimitedRosterMode()
        );
        ...
        const landingUiLocked = !!(typeof game !== 'undefined' && game && game._landingSpawnUiLocked === true);
    ...
    updateCategoryTab(currentCategory) {
        ...
        let selected = (currentCategory === 'infantry' || currentCategory === 'armored' || currentCategory === 'air')
            ? currentCategory
            : 'infantry';
        if (g) g.currentCategory = selected;

        const landingUiLocked = !!(g && g._landingSpawnUiLocked === true);
        const tabs = [
        ...
        tabs.forEach(({ id, key }) => {
            ...
            if (!el) return;
            const showTab = !landingUiLocked;
            const nextDisplay = showTab ? '' : 'none';
