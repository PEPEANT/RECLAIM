...
const CanvasRenderer = {
    ...
    /**
     * Draw in edit mode (original logic)
     */
    drawEditMode() {
        const { ctx, canvas } = this;
    ...
    /**
     * Draw in preview mode (in-game rendering)
     */
    drawPreviewMode() {
        const { ctx, canvas } = this;
        const unitId = EditorState.currentUnitId;
        const { team, facing, unitMode } = EditorState.previewState;
        const rotorAngle = EditorState.rotorAngle;
        const scale = EditorState.scale;
        ...
        // Draw preview info header
        this.drawPreviewHeader(ctx, canvas, unitId, team);
        ...
        ctx.textAlign = 'right';
        ctx.fillText('GROUND', 145, 12 / scale);

        // Draw unit using IngameRenderer if available
        if (typeof IngameRenderer !== 'undefined' && unitId) {
            IngameRenderer.draw(ctx, unitId, {
                team,
                facing,
                rotorAngle,
                mode: unitMode
            ...
            ctx.fillText(unitId, 0, -35);
        }

        ctx.restore();

        // Draw preview mode indicator
        this.drawPreviewIndicator(ctx, canvas);
    },

    /**
     * Draw preview header with unit info
     */
    drawPreviewHeader(ctx, canvas, unitId, team) {
        const unitData = EditorState.unitData;
        const unitName = unitData?.name || unitId || '유닛 미선택';
        ...
        ctx.textAlign = 'left';
        ctx.fillText(unitName, 20, 30);

        // Team indicator
        const teamColor = team === 'player' ? '#3b82f6' : '#ef4444';
        const teamLabel = team === 'player' ? 'PLAYER' : 'ENEMY';
        ctx.fillStyle = teamColor;
        ctx.beginPath();
        ctx.arc(canvas.width - 80, 25, 8, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#94a3b8';
        ctx.font = '12px sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(teamLabel, canvas.width - 65, 29);
    },

    /**
     * Draw preview mode indicator
     */
    drawPreviewIndicator(ctx, canvas) {
        ctx.fillStyle = '#22c55e';
        ctx.font = 'bold 11px sans-serif';
        ctx.textAlign = 'right';
        ctx.fillText('● 미리보기 모드', canvas.width - 15, canvas.height - 15);
    },
