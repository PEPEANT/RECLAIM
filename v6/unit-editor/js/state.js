...
const EditorState = {
    ...
    previewState: {
        ...
        soundEnabled: true,    // MP3 ON/OFF
        unitMode: null,        // 2모드 유닛: 'rifle'|'laptop' 또는 'carrying'|'firing'
        facing: 1,             // 1 또는 -1
    ...
    /**
     * Set preview team color
     * @param {string} team - 'player' or 'enemy'
     */
    setPreviewTeam(team) {
        if (team !== 'player' && team !== 'enemy') return;
        this.previewState.team = team;
        this.emit('previewStateChanged', this.previewState);
    },

    /**
     * Set unit mode for 2-mode units (engineer, drone_operator)
     * @param {string} mode - Mode name
     */
    setPreviewUnitMode(mode) {
        this.previewState.unitMode = mode;
        this.emit('previewStateChanged', this.previewState);
    },

    /**
     * Toggle preview sound
     */
    togglePreviewSound() {
        this.previewState.soundEnabled = !this.previewState.soundEnabled;
        this.emit('previewStateChanged', this.previewState);
    },

    /**
     * Set preview facing direction
     * @param {number} facing - 1 or -1
     */
    setPreviewFacing(facing) {
        this.previewState.facing = facing === -1 ? -1 : 1;
        this.emit('previewStateChanged', this.previewState);
    },

    /**
     * Check if current unit has multiple modes
     * @returns {Object|null} Mode info or null
     */
    getUnitModeInfo() {
        const unitId = this.currentUnitId;
        if (!unitId) return null;

        const TWO_MODE_UNITS = {
            'drone_operator': {
                modes: ['rifle', 'laptop'],
                labels: ['소총 모드', '노트북 모드'],
                default: 'rifle'
            ...
            'engineer': {
                modes: ['carrying', 'firing'],
                labels: ['LMG 모드', 'RPG 모드'],
                default: 'carrying'
            },
            'rpg': {
                modes: ['carrying', 'firing'],
        ...
        return TWO_MODE_UNITS[unitId] || null;
    },
