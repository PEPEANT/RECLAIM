...
const UnitLoader = {
     ...
     */
    async loadIndex() {
        const fromConfig = this.buildIndexFromConfig();
        if (fromConfig) {
            this.unitList = fromConfig.units;
    ...
    /**
     * Build unit index from in-game CONFIG.units (hardcoded source)
     * @returns {Object|null} Unit index data
     */
    buildIndexFromConfig() {
        if (typeof CONFIG === 'undefined' || !CONFIG || !CONFIG.units) return null;

        const units = Object.values(CONFIG.units)
            .filter(u => u && u.id)
            .filter(u => u.hideFromUnitBar !== true)
            .filter(u => u.hideFromUnitDex !== true)
            .filter(u => u.disabled !== true)
            .filter(u => u.isSkill !== true)
            .filter(u => u.droneLaunchOnly !== true)
            .map(u => ({
                id: u.id,
                name: u.name || u.id,
                type: this.mapCategoryToType(u.category)
            }));

        if (units.length === 0) return null;
        return { version: "game", source: "CONFIG.units", units };
    },

    /**
     * Map in-game category to editor type
     * @param {string} category - Unit category
     * @returns {string} Editor unit type
     */
    mapCategoryToType(category) {
        if (category === 'armored') return 'vehicle';
        if (category === 'air') return 'air';
        if (category === 'drone') return 'drone';
        return 'infantry';
    },
    ...
    getFallbackIndex() {
        const fallback = {
            version: "2.0",
            units: [
                { id: "worker", name: "작업자", type: "infantry" },
                { id: "infantry", name: "보병", type: "infantry" },
                { id: "engineer", name: "공병", type: "infantry" },
                { id: "special_forces", name: "특수부대", type: "infantry" },
                { id: "humvee", name: "험비", type: "vehicle" },
                { id: "apc", name: "장갑차", type: "vehicle" },
                { id: "aa_tank", name: "대공전차", type: "vehicle" },
                { id: "mbt", name: "전차", type: "vehicle" },
                { id: "spg", name: "자주포", type: "vehicle" },
                { id: "fighter", name: "전투기", type: "air" },
                { id: "apache", name: "아파치", type: "air" },
                { id: "blackhawk", name: "수송헬기 UH-60", type: "air" },
                { id: "chinook", name: "치누크", type: "air" },
                { id: "bomber", name: "폭격기", type: "air" },
                { id: "recon", name: "정찰기", type: "air" },
                { id: "drone_operator", name: "드론병", type: "infantry" }
            ]
        };
