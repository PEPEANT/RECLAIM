...
class Unit extends Entity {
        ...
        if (attacker && this.attackTarget && !this.attackTarget.stats) {
            if (attacker !== this.attackTarget && !attacker.dead) {
                ...
                if (attackerTeam && attackerTeam !== this.team && attackerTeam !== 'neutral') {
                    ...
                    const canHitInvuln = (this.stats.id === 'aa_tank' && attacker.stats && attacker.stats.id === 'chinook');
                    if ((!attackerIsAir || canHitAir) && (!attackerInvuln || canHitInvuln)) {
    ...
    update(enemies, buildings) {
        ...
        if (this.attackTarget && (this.attackTarget.dead ||
            (this.attackTarget.stats && this.attackTarget.stats.invulnerable && !(this.stats.id === 'aa_tank' && this.attackTarget.stats.id === 'chinook')) ||
        ...
        if (!this.attackTarget) {
            ...
            if (game.frame >= this.nextTargetScanFrame) {
                ...
                const allowInvulnerableAir = (this.stats.id === 'aa_tank');
                for (let e of enemies) {
                    if (!e || e.dead) continue;
                    if (e.stats && e.stats.stealth) continue;
                    if (onlyAir && (!e.stats || e.stats.type !== 'air')) continue;
                    if (e.stats && e.stats.invulnerable && !(allowInvulnerableAir && e.stats.id === 'chinook')) continue;
    ...
    _startTankMGSound() {
        ...
        try {
            ...
            const vol = AudioSystem.volume.sfx * AudioSystem.volume.master * 0.5;
                ...
                if (p && p.catch) {
                    p.catch(() => {
                        if (AudioSystem._playOneShot) {
                            AudioSystem._playOneShot('bgm/machine_gun2.mp3', vol, 6);
                        }
                    });
                }
            }
    ...
    attack(target) {
        ...
        if (target && target.stats && target.stats.invulnerable) {
            if (!(this.stats.id === 'aa_tank' && target.stats.id === 'chinook')) return;
        }
        const id = this.stats.id;
        let dmg = this.stats.damage;
        ...
        if (!skipGunSFX && typeof AudioSystem !== 'undefined' && Math.random() < 0.3) {
        ...
        if (id === 'apache' && type === 'rocket') {
            this.missileFlash = 6;
            if (typeof AudioSystem !== 'undefined') {
                AudioSystem.playSFX('apache_missile');
            }
        }
