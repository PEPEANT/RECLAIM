...
const game = {
    ...
    applyVersionLabels() {
        ...
        if (brandEl) brandEl.textContent = v.brand || v.brandName || 'RECLAIM';
    ...
    loop() {
        ...
        let updates = 1;
        if (this.paused) updates = 0;
        else if (this.speed === 2) updates = 2;
    ...
    update() {
        ...
        if (this.civilianGlobalPanic > 0) {
            this.civilianGlobalPanic--;
        }
        ...
        if (typeof GameNews !== 'undefined' && GameNews.update) {
            GameNews.update(this, elapsedSeconds, playerHQ);
        }

        if (typeof GameVictory !== 'undefined' && GameVictory.check) {
            GameVictory.check(this, elapsedSeconds, playerHQ, alivePlayerUnits, aliveEnemyUnits);
        }
        if (this.isGameOver) return;

        // [P1] 시체 생성 버스트 완화: 프레임당 예산만큼 처리
        this._processCorpseSpawnQueue();
        ...
        // [NEW] 잔해 업데이트
        this.wreckages.forEach(w => w.update());
        // [NEW] 보병 시체 업데이트
        ...
        compactArray(this.wreckages, w => w.life > 0);
        compactArray(this.corpses, c => {
            if (!c) return false;
            if (Number.isFinite(c.deadAt) && Number.isFinite(this.frame)) {
                return (this.frame - c.deadAt) < c.fadeTimer;
            }
            return c.fadeTimer > 0;
        });
    ...
    draw() {
        ...
        if (typeof Maps !== 'undefined') {
            ...
            Maps.drawBase(ctx, this.width, this.height, this.groundY, this.cameraX);
        ...
        const doCorpseProfile = !!(this.debug && this.debug.corpseProfile && typeof performance !== 'undefined' && performance.now);
        const corpseProfileEvery = (this.debug && Number.isFinite(this.debug.corpseProfileEvery) && this.debug.corpseProfileEvery > 0)
            ? this.debug.corpseProfileEvery
            : 120;
        const corpseDrawStart = doCorpseProfile ? performance.now() : 0;
        const z = (typeof Camera !== 'undefined' && Camera.zoom) ? Camera.zoom : 1;
        const pad = Number.isFinite(this.corpseCullPadding) ? this.corpseCullPadding : 0;
        const viewLeft = this.cameraX - pad / z;
        const viewRight = this.cameraX + (this.width + pad) / z;
        const viewTop = this.groundY + ((-pad - this.groundY) / z);
        const viewBottom = this.groundY + ((this.height + pad - this.groundY) / z);
        this.corpses.forEach(c => {
            if (!c) return;
            if (c.x < viewLeft || c.x > viewRight || c.y < viewTop || c.y > viewBottom) return;
            c.draw(ctx);
        });
        if (doCorpseProfile && (this.frame % corpseProfileEvery === 0)) {
            const dt = performance.now() - corpseDrawStart;
            console.log(`[perf] corpses=${this.corpses.length} draw=${dt.toFixed(2)}ms`);
        }
