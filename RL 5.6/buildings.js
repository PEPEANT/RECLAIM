...
class Building extends Entity {
    constructor(type, x, y, team) {
        ...
        this.garrisonUnits = [];  // [NEW] 여러 유닛 주둔 가능 (최대 7기)
        this.maxGarrison = 7;     // [NEW] 최대 주둔 가능 보병 수
    ...
    takeDamage(amount, attackType = null) {
        ...
        const explosiveAttacks = ['artillery', 'bomb', 'nuke', 'tactical_missile', 'rocket', 'shell', 'engineer_missile', 'drone_explosion', 'humvee_burst'];
        ...
        if (this.type === 'bunker') {
            ...
            if (this.hp <= 0) {
                ...
                if (isExplosive && this.team !== 'neutral') {
    ...
    update(enemies, players) {
        ...
        // [NEW] 거점(벙커) garrison: 최대 7기 보병 주둔 가능
    ...
    ejectAllGarrison() {
        ...
        for (let i = 0; i < this.garrisonUnits.length; i++) {
            ...
            const offsetAmount = Math.floor((i + 1) / 2) * spreadOffset;
            unit.x = this.x + offsetDir * (this.width / 2 + 20 + offsetAmount);
            unit.y = groundY;  // [FIX] 지면에 배치
    ...
    draw(ctx) {
        ...
        if (this.type.includes('hq')) {
            ...
            const hasCityDesign = isCity && (cityVariant === 'luxury' || cityVariant === 'legacy')
                && (typeof Maps !== 'undefined') && (typeof Maps.drawLuxuryApartment === 'function');
            ...
            if (hasCityDesign) {
                ...
                if (this.isDestroyed) {
                    const drawDestroyedFn = (cityVariant === 'luxury')
                        ? Maps.drawDestroyedLuxuryApartment
                        : Maps.drawDestroyedLegacyApartment;
                    if (drawDestroyedFn) {
                ...
                const drawFn = (cityVariant === 'luxury') ? Maps.drawLuxuryApartment : Maps.drawLegacyApartment;
