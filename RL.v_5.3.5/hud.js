...
const HUD = {
    ...
    updateCommandButtons() {
        ...
        if (hasSelection) {
            ...
            stateKey += `|recon:${hasRecon}|sf:${hasSF}|smoke:${hasSmokeCharge}`;
        }
        if (game.selectedBuilding && game.selectedBuilding.type === 'bunker') {
            const b = game.selectedBuilding;
    ...
    setSelection(selection) {
        ...
        if (selection.kind === 'unit') {
            ...
            if (isHQ) {
                ...
                let statusHtml = '';
                if (isDestroyed) {
                    statusHtml = '<span class="text-red-500 font-bold">🏚️ 파괴됨 (재점령 불가)</span>';
                } else if (garrisonCount > 0 && b.team === 'player') {
                    // [NEW] 주둔 보병 종류별 그룹화
                    const unitGroups = {};
    ...
    drawMinimap() {
        ...
        const hash = `${game.buildings.length}|${game.players.length}|${game.enemies.length}|${camX}`;
        if (hash === this._lastMinimapHash) return;
