...
// - game.particles ?�이?�라???�드 좌표)�??�환: update(), draw(ctx), life
// - ?�면 ?�래???�들림�? game.addFlash / game.addShake �??�결
...
(function () {
    ...
    // game.particles 과다 ?�적 방�? (최우?�으�??�순 Particle ?�거)
    function pruneParticles(game, limit) {
        ...
        // 1) 가???�래??것�???"?�순 Particle" ?�주 ?�거
        //    (classes.js Particle: 보통 kind ?�음)
    ...
    // ==========================
    // Presets (?�기/강도 ?�동 조절?� spawn?�서 scale�?
    // ==========================
    const PRESETS = {
        nuke: {
            life: 2.2,
    ...
    const VFX = {
        ...
        spawnExplosion(game, kind, x, y, opts) {
            ...
            const anchorGround = (opts && typeof opts.anchorGround === 'boolean')
                ? opts.anchorGround
                : (kind === 'nuke' || kind === 'bomb' || kind === 'artillery' || kind === 'stealth' || kind === 'vehicle' || kind === 'tactical');
