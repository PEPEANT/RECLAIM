const AI = {
    ...
    _getTimeBasedSequence(frame) {
        ...
        if (sec < 180) {
            return { key: '01:00-03:00', sequence: ['infantry', 'mbt', 'aa_tank', 'infantry', 'aa_tank'] };
        }
        if (sec < 300) {
        ...
        return { key: '07:00+', sequence: ['bomber', 'fighter', 'apache', 'apc', 'mbt', 'aa_tank', 'spg', 'infantry', 'engineer', 'humvee', 'special_forces', 'rpg', 'drone_operator'] };
    },
    ...
    getCounterUnit(info, isElite) {
        ...
        if (isMid) {
            ...
            // 기본 중반 풀 (기갑 포함, 공중 제외)
            const midPool = ['infantry', 'engineer', 'humvee', 'mbt', 'aa_tank'];
            if (isElite) {
                const eliteMidPool = ['infantry', 'engineer', 'mbt', 'aa_tank', 'spg', 'special_forces'];
