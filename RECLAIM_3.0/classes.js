    ...
    update(enemies, buildings) {
        ...
        if (['blackhawk', 'chinook'].includes(this.stats.id)) {
            ...
            if (this.stats.id === 'chinook') {
                ...
                return; // 移섎늻?щ뒗 怨듦꺽 ?덊븿
            }

            // [?섏젙] 釉붾옓?명겕: ?대룞 -> ?ы븯 -> **?꾪닾 紐⑤뱶 ?꾪솚**
            if (this.stats.id === 'blackhawk') {
                if (!this.deployed) {
                    ...
                    if (dist < 20) {
                        ...
                        // [以묒슂] ?ы븯 ??利됱떆 怨듦꺽 ?寃?李얜룄濡?由ъ뀑
                        this.attackTarget = null;
                    } else {
                        this.x += (dx / dist) * speed;
                        this.y += (dy / dist) * (speed * 0.8);
                        return; // ?대룞 以묒뿉??怨듦꺽 ?덊븿
                    }
                }
                // deployed === true硫??꾨옒 ?쇰컲 ?꾪닾 濡쒖쭅?쇰줈 ?섏뼱媛?(怨듦꺽 ?ш린濡?蹂??
            }
        }
